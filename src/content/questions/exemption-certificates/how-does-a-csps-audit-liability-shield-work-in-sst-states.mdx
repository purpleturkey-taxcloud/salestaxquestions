...
tldr: "In all 24 SST member states, when a seller uses a CSP and the CSP calculates tax incorrectly, the liability shifts to the CSP — not the seller — as long as the seller provided accurate transaction data. The shield does not cover seller-side errors: wrong product tax codes, missing nexus registrations, or invalid exemption certificates remain the seller's liability."
faqs:
