...
tldr: "Yes — a 501(c)(3) IRS determination letter is not a sales tax exemption certificate and is not sufficient on its own. Each state issues its own exemption documentation, and nonprofits must apply separately in each state. Collect the state-issued certificate before exempting any sale; if it's not on file, collect tax."
faqs:
