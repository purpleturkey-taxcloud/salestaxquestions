...
tldr: "Unregistered nexus across 10+ states is a manageable problem if you act before any state contacts you first. A coordinated multi-state VDA — ideally through the MTC's program, which covers 39 states in a single anonymous application — compresses the lookback to 3–4 years and waives penalties; the process typically takes 4–8 months from engagement to resolution. The window closes the moment a state sends a nexus questionnaire."
faqs:
