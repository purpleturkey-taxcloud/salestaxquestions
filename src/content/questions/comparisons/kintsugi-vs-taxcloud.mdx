...
faqs:
  ...
  - question: "Has Kintsugi had support issues?"
    answer: "Kintsugi runs an AI-first, platform-led model that is lighter on human-support depth than providers built around US-based human filing teams. That trade-off is most noticeable as state footprints grow and edge cases — amendments, state notices, non-standard use cases — increase. Buyers should ask specifically about support response times, who handles state correspondence, and what happens when amendments or state notices arrive."
...
**US-based human support.** 41% of TaxCloud's G2 reviews specifically call out US-based support as a differentiator, reviewers describe "real Americans handling phone calls and emails" and teams that "went above and beyond." On the Shopify App Store, TaxCloud holds [4.8 stars across 97 reviews](https://apps.shopify.com/taxcloud-sales-tax-automation/reviews), 93 of them five-star. For sellers who've been through support hell at Avalara or TaxJar, this matters — and it's the axis where Kintsugi's lighter-touch, AI-first support model is most likely to show a gap, particularly at higher state counts.
...
## How Kintsugi's model shapes the experience

Kintsugi is AI-native and Stripe-first, purpose-built for SaaS and digital-goods companies on modern billing stacks. That design shows up as real strengths: fast setup, a clean QuickBooks and Stripe integration, and a well-regarded interface. Its 4.8/5 G2 rating reflects a product users find genuinely easy to work in.

The structural trade-off of an AI-first, platform-led model is human-support depth. Kintsugi is not a CSP, and its support model is lighter-touch than a provider built around US-based human filing support. For SaaS-first companies at moderate state counts, that trade-off rarely bites — the product does what it promises.

As state footprints grow and edge cases multiply — amendments, state notices, non-standard use cases — the value of reaching a person who owns the issue goes up. That is the axis to weigh Kintsugi on: not whether the product is well-built (it is), but whether your state count and complexity will outrun a lighter-touch support model.
...
Kintsugi's AI-first, platform-led model is lighter on human-support depth than providers built around US-based human filing teams: a fast-shipping, well-designed product, with a thinner human layer behind it. That trade-off is most noticeable as state footprints grow and edge cases — amendments, state notices, non-standard use cases — increase. Buyers should ask specifically about support response times, who handles state correspondence, and what happens when amendments or state notices arrive.
...
This comparison draws on published TaxCloud and Kintsugi pricing, Kintsugi's public G2 reviews, and the Streamlined Sales Tax Governing Board's [public Certified Service Provider roster](https://www.streamlinedsalestax.org/certified-service-providers/certified-service-providers-list). Pricing and CSP status verified as of May 2026.
